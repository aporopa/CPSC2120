...
void ListQueue<T>::enqueue(T value){
  ...
  //putting value at end of the list
  this->queue.insertEnd(value);
}
...
T ListQueue<T>::dequeue(){
  //grabs the size of the queue
  int size = this->queue.size();

  //because queues are FIFO, grab at position 0
  T variable = this->queue.getAt(0);
