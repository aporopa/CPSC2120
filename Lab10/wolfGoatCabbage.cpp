...
void build_graph(void) {
    //creating maximum bit 2^4
    const int MAX_BIT = 1 << 4;
    //driving loop to find all possible combinations
    for (int i = 0; i < MAX_BIT; i++) {
        vector<int> contains;
        //loop through possible neighbors
        for (int j = 0; j < MAX_BIT; j++) {
            const string str = neighbor_label(i, j);
            //create an edge if necessary
            if (!str.empty() && j != 3 && j != 12 && j != 6 && j != 7 && j != 8 && j != 9) {
                edge_label.emplace(make_pair(i, j), str);
                contains.push_back(j);
            }
        }
        //add the vector to nbrs with i as the key
        nbrs.emplace(i, move(contains));
    }
}
